  ...
  class ProfileModificationRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CommafeedClient::ProfileModificationRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CommafeedClient::ProfileModificationRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'new_api_key')
        self.new_api_key = attributes[:'new_api_key']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @current_password.to_s.length > 128
        invalid_properties.push('invalid value for "current_password", the character length must be smaller than or equal to 128.')
      end

      if @current_password.to_s.length < 0
        invalid_properties.push('invalid value for "current_password", the character length must be great than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if @current_password.to_s.length > 128
      return false if @current_password.to_s.length < 0
    ...
    def current_password=(current_password)
      ...
      if current_password.to_s.length > 128
        fail ArgumentError, 'invalid value for "current_password", the character length must be smaller than or equal to 128.'
      end

      if current_password.to_s.length < 0
        fail ArgumentError, 'invalid value for "current_password", the character length must be great than or equal to 0.'
      end
    ...
    def hash
      [current_password, email, new_password, new_api_key].hash
    end
