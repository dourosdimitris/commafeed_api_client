  ...
  class SubscribeRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CommafeedClient::SubscribeRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CommafeedClient::SubscribeRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @url.to_s.length > 4096
        invalid_properties.push('invalid value for "url", the character length must be smaller than or equal to 4096.')
      end
    ...
    def valid?
      ...
      return false if @url.to_s.length > 4096
    ...
    def url=(url)
      ...
      if url.to_s.length > 4096
        fail ArgumentError, 'invalid value for "url", the character length must be smaller than or equal to 4096.'
      end
